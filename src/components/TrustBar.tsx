        ...
        <div className="relative overflow-hidden">
          ...
          >
            {[...clients, ...clients].map((client, index) => (
              <motion.div
                key={index}
                className="flex-shrink-0 flex items-center justify-center h-12 sm:h-16 px-4 sm:px-8 cursor-pointer group"
                whileHover={{ scale: 1.1 }}
              >
                <span className="text-sm sm:text-base lg:text-lg font-semibold text-muted-foreground whitespace-nowrap group-hover:text-primary transition-colors duration-300">
                  {client}
                </span>
              </motion.div>
            ))}
